...
script.api.create = function (positionArray, colorArray) {
  ...
  print(
    "object sent (script.api.create): position = " +
      positionArray +
      ", colorArray = " +
      colorArray
  );
...
// * My code
script.api.testPoker = function (msgStr) {
  print("poker test line");
  packets.sendObject("/poker/test/", {
    msg: { hello: msgStr },
  });
};

packets.on("/poker/test/", function (body, params, userId) {
  print("recieved /poker/test/. Body: ", body);
  var message = JSON.parse(body);
  events.trigger("pokerTest", message.hello);
});
// * End my code

// Handle incoming messages
packets.on("/blocks/create/", function (body, params, userId) {
  print("recieved create", body);
  var message = JSON.parse(body);
...
function requestPlayersCache() {
  ...
  packets.on(
    "/blocks/state/offer/" + requestId + "/",
    handleStateShareOffer
  );
  ...
  function handleStateShareOffer(offerToken) {
    ...
    packets.on(
      "/blocks/state/create/" + offerToken + "/",
      function (body, params, userId) {
        var message = JSON.parse(body);
        print("received message from another: " + message);
        log("Createing block from players state");
        events.trigger("create", message.p, message.c);
        events.trigger("pokerTest", { hello: "world" }); // ! test
      }
    );
