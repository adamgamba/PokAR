...
script.api.start = function () {
  ...
  function onUpdate(eventData) {
    ...
    print("onUpdate()..."); // ! runs very often
    ...
    if (script.isTouching) {
      ...
      print("script.numFramesTouchDown =" + script.numFramesTouchDown);
    }
    ...
    if (script.numFramesTouchDown > 1) {
      if (!isOccupied(p1Quantized, script.occupancy)) {
        ...
        print("run script.blockSync.api.create...");
        script.blockSync.api.create(positionArray, colorArray); // Tell others
        script.blockSync.api.testPoker("hello World"); // ! test
  ...
  function createBlock(positionArray, colorArray) {
    print("BlockDrawing.createBlock()...");
  ...
  // * My code
  script.blockSync.api.events.on("pokerTest", function (msg) {
    print("pokerTest event ran: msg=", msg);
  });
