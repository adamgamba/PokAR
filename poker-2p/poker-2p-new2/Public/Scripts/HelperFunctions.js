...
script.api.getScreenCapture = function (captureMeshes) {
  ...
  script.captureUnlitMaterial.mainPass.baseTex =
    script.captureCameraOutput;
...
script.api.getDistanceFromCamera = function (objectDistanceToCamera) {
  if (objectDistanceToCamera && script.camera) {
    var pointA = objectDistanceToCamera
      .getTransform()
      .getWorldPosition();
    ...
    print(
      "HelperFunctions: Please assign camera and the Object to the script"
    );
  }
};
