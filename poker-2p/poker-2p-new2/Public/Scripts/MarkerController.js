...
script.marker.onMarkerFound = wrapFunction(
  script.marker.onMarkerFound,
  function () {
    ...
    if (markerObject) {
      for (var i = 0; i < markerObject.getChildrenCount(); i++) {
        ...
        var scriptComponents = childObject.getComponents(
...
script.marker.onMarkerLost = wrapFunction(
  script.marker.onMarkerLost,
  function () {
    if (script.hintControllerScript) {
