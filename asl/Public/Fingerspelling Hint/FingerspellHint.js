...
function playAnim(layerName, letter) {
    ...
    if (layerNamesAvailable.indexOf(layerName) < 0) {
        print("Warning, Hint animation is not implemented for character \"" + letter + "\"");
        return;
    }
...
function checkInputs() {
    if (!script.mixer) {
        print("ERROR, Animation Mixer input is not set on" + script.getSceneObject().name);
        return false;
    }
    if (!script.hintText) {
    ...
    if (!script.objectTracking) {
        print("ERROR, Hand Tracking input is not set on " + script.getSceneObject().name);
        return false;
    }
    return true;
}
...
// show letter
script.showLetter = function(c) {
    anim.nextLetter = c.toUpperCase();
};
// update hand side
script.checkSide = checkSide;
