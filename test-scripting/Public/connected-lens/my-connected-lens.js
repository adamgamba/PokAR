...
function shareSession() {
  ...
  function onSessionShared(session, snapcodeTexture) {
    ...
    if (
      invitationType === ConnectedLensModule.SessionShareType.Snapcode
    ) {
  ...
  // Request to Share
  script.connectedLensModule.shareSession(
    invitationType,
    onSessionShared
  );
}
...
function onSessionCreated(session, sessionCreationType) {
  ...
  if (
    ...
    sessionCreationType ==
    ConnectedLensSessionOptions.SessionCreationType.New
  ) {
...
function onUserJoinedSession(session, userInfo) {
  if (script.onUserJoinedSession)
    script.onUserJoinedSession(session, userInfo);
}
...
function onError(session, errorCode, description) {
  print(
    "[Connected Lens Session Error] " + errorCode + ": " + description
  );
}
...
function onStarterConnectedToSolo(session) {
  if (script.onStarterConnectedToSolo)
    script.onStarterConnectedToSolo(session);
}

function onStarterConnectedToMultiplayer(session) {
  if (script.onStarterConnectedToMultiplayer)
    script.onStarterConnectedToMultiplayer(session);
}

function onReceiverConnectedToMultiplayer(session) {
  if (script.onReceiverConnectedToMultiplayer)
    script.onReceiverConnectedToMultiplayer(session);
}
...
function onRealtimeStoreCreated(session, store, userInfo) {
  if (script.onRealtimeStoreCreated)
    script.onRealtimeStoreCreated(session, store, userInfo);
}

function onRealtimeStoreUpdated(session, store, userInfo) {
  if (script.onRealtimeStoreUpdated)
    script.onRealtimeStoreUpdated(session, store, userInfo);
}

function onRealtimeStoreDeleted(session, store) {
  if (script.onRealtimeStoreDeleted)
    script.onRealtimeStoreDeleted(session, store);
}

function onRealtimeStoreOwnershipUpdated(session, store, ownerInfo) {
  if (script.onRealtimeStoreOwnershipUpdated)
    script.onRealtimeStoreOwnershipUpdated(session, store, ownerInfo);
}
...
//print("test")
//script.connectedLensModule.createSession();
